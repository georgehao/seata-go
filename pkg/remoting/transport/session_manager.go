...
func (g *SessionManager) selectSession(msg interface{}, loadBalanceType string) getty.Session {
	session := loadbalance.Select(loadBalanceType, &g.allSessions, g.getXid(msg))
	if session != nil {
		return session
	}
	if g.sessionSize == 0 {
