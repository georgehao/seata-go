...
import (
	...
	"github.com/seata/seata-go/pkg/datasource/exec"
)
...
type Stmt struct {
	...
	res *DBResource
	// txCtx
	txCtx *schema.TransactionContext
	// query
	query string
	// stmt
	stmt driver.Stmt
...
func (s *Stmt) Query(args []driver.Value) (driver.Rows, error) {
	...
	ret, err := executor.ExecWithValue(context.Background(), execCtx,
		func(ctx context.Context, query string, args []driver.Value) (schema.ExecResult, error) {
			ret, err := s.stmt.Query(args)
			if err != nil {
				return nil, err
			}

			return schema.NewResult(schema.WithRows(ret)), nil
		})
	if err != nil {
		return nil, err
	}
...
func (s *Stmt) QueryContext(ctx context.Context, args []driver.NamedValue) (driver.Rows, error) {
	...
	ret, err := executor.ExecWithNamedValue(context.Background(), execCtx,
		func(ctx context.Context, query string, args []driver.NamedValue) (schema.ExecResult, error) {
...
func (s *Stmt) Exec(args []driver.Value) (driver.Result, error) {
	...
	ret, err := executor.ExecWithValue(context.Background(), execCtx,
		func(ctx context.Context, query string, args []driver.Value) (schema.ExecResult, error) {
			...
			return schema.NewResult(schema.WithResult(ret)), nil
		})
