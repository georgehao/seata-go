...
import (
	...
	"github.com/seata/seata-go/pkg/datasource/mock"
...
func Test_seataATConnector_Connect(t *testing.T) {
	...
	assert.True(t, atConn.txType == schema.Local, "init need local tx")
}
...
func Test_seataXAConnector_Connect(t *testing.T) {
	...
	assert.True(t, xaConn.txType == schema.Local, "init need local tx")
}
